...
public interface UserDao {
    ...
    //文件分页接口
    @Select("select a.*,b.name username from test_file a left join test_user b on a.user = b.id limit #{pageNo},#{pageSize}" )
    List<File> getFilePage(@Param("pageNo") int pageNo,@Param("pageSize") int pageSize);

    @Select("select count(*) from test_file a left join test_user b on a.user = b.id")
    int getTotal();
