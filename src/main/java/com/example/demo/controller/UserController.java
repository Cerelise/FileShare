...
//    }
    ...
    @PostMapping("/inline")
    @ResponseBody
    public Result inline(HttpServletRequest request) {
        HttpSession session = request.getSession();
        User u = (User) session.getAttribute("user");
        if (u.getRole()==0){
            return Result.success("",userService.inline());
        ...
        //普通用户处理
        List<User> inline = userService.inline();
        ListIterator<User> userListIterator = inline.listIterator();
        while (userListIterator.hasNext()){
            User user = userListIterator.next();
            user.setPassword("****无权查看****");
        }
        return  Result.success("普通用户无权限查看密码",inline);
    }
    ...
    @ResponseBody
    public Result getFilePage(@Param("pageNo") int pageNo,@Param("pageSize") int pageSize){
        pageNo = (pageNo-1)*pageSize;
        List<File> filePage = userService.getFilePage(pageNo, pageSize);
        System.out.println(filePage);
        int total = userService.getTotal();
        return Result.success(total + "",filePage);
    }
